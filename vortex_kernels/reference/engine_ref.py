"""
Verbatim copy of vortex/model/engine.py at commit cb229ae.

Do NOT modify — these are golden references for correctness tests.
Update only by re-vendoring from a newer vortex commit.

Sourced from:
    .venv/lib/python3.12/site-packages/vortex/model/engine.py
on 2026-05-10. Verify the upstream SHA matches before bumping.

Original copyright: (c) 2024, Michael Poli. Vendored under the same terms
as the upstream vortex repository.
"""
...
class HyenaInferenceEngine:
    def __init__(
        ...
        self.fir_fn = fir_fn
        assert iir_prefill_style in IIR_PREFILL_MODES, (
    ...
    def prefill_via_direct_recurrence(
        ...
        for i in range(L):
            state[..., 0] = (
                poles[..., 0] * state[..., 0] - poles[..., 1] * state[..., 1] + x1v_[:, :, i, :, 0]
            )
            state[..., 1] = (
                poles[..., 0] * state[..., 1] + poles[..., 1] * state[..., 0] + x1v_[:, :, i, :, 1]
            )
